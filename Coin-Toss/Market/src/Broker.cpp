...
Market::Broker::Broker(const std::string& brokerID, uint32_t traderNum, const std::vector<Stock::Stock*>& stocks): b_brokerID(brokerID){
	b_traders.reserve(traderNum);
	for (uint32_t i = 0; i < traderNum; i++) {
