...
Stock::CandleStick::CandleStick(float openingPrice) : cs_maxPrice(openingPrice), cs_minPrice(openingPrice), cs_openPrice(openingPrice), cs_closePrice(openingPrice) {}

Stock::CandleStick::CandleStick(float max_price, float min_price) : cs_maxPrice(max_price), cs_minPrice(min_price), cs_openPrice(0), cs_closePrice(0) {}
...
static HighLowPrice traded_price(const Stock::StockQue& current, const Stock::StockQue& previous) {
	...
	if (!buy_pdiff && !sell_pdiff) {
		...
		uint32_t buy_vdiff = (uint32_t)(current.mq_topVol_B[0] - previous.mq_topPrice_B[0]);
		uint32_t sell_vdiff = (uint32_t)(current.mq_topVol_S[0] - previous.mq_topPrice_S[0]);
		if (buy_vdiff < 0) {
	...
	//if price changes
		...
		if (buy_pdiff > 0 && sell_pdiff > 0) {
			...
			for (uint32_t i = 1; i < previous.mq_size; i++) {
