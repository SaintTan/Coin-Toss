...
float Analysis::BasicAnalysis::priceChanges(const Stock::StockQue& current, const Stock::StockQue& previous, const std::string& mode) {
	if (mode == "buy") return previous.mq_topPrice_B[0] - current.mq_topPrice_B[0];
	else if (mode == "sell") return previous.mq_topPrice_S[0] - current.mq_topPrice_S[0];
	else return 0; 
}

unsigned int Analysis::BasicAnalysis::volChanges(const Stock::StockQue& current, const Stock::StockQue& previous, const std::string& mode) {
